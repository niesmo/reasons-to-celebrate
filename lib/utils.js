...
function isNumberSpecial(num){
    ...
    // 6. sorounded by same numbers 14441, 8118 ✔
    ...
    return sameDigits(numStr) 
        || repeatingDigits (numStr) 
        || increasingOrDecreasingDigits(numStr)
        || soroundedBySameDigis(numStr)
        || roundNumber(numStr)
        || mirrorDigits(numStr)
}
...
function soroundedBySameDigis (numStr){
    if (numStr.length < 4) return false
    let [first, last] = [numStr[0], numStr[numStr.length - 1]]

    return first === last && sameDigits(numStr.slice(1,numStr.length - 1))
}

function roundNumber (numStr){
    if(numStr.length < 3) return false
    return +numStr % Math.pow(10,Math.floor(Math.log10(+numStr))) === 0
}

function mirrorDigits (numStr){
    if(numStr.length < 3) return false
    let rev = ""
    for(let i=numStr.length-1;i>=0;i--) rev += numStr[i]
    return numStr === rev
}