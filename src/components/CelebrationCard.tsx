...
const CelebrationCard: React.FC<CelebrationCardProps> = ({ celebration, sourceName }) => {
    ...
    return (
        <div className={`relative rounded-xl p-6 text-white shadow-lg transition-transform hover:scale-105 bg-gradient-to-br ${getGradient(type)} group`}>
            ...
            <div className="relative z-10">
                ...
                <div className="flex gap-2">
                    ...
                    <ReminderButton
                        celebrationDate={celebration.originalDate}
                        celebrationTargetDate={date.toISOString()}
                        daysCount={daysFromStart}
                        celebrationName={`${daysFromStart.toLocaleString()} days - ${type}`}
                    />
                </div>
            </div>
        </div>
